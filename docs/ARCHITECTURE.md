...
| Milo / RAG / classification | **PawBuck.API** + **Supabase Edge** | Split by runtime (see **Milo runtime** below). |

## Milo runtime (chat vs FAQ RAG)

**In-app conversational Milo** (Expo consumer) calls the Supabase Edge Function **`milo-chat`** only — see [`apps/consumer-app/context/chatContext.tsx`](../apps/consumer-app/context/chatContext.tsx). That function runs Gemini with function tools: FAQ search, vaccine laws, pet health reads, and **curated guidance** snippets (see [`docs/MILO_RAG.md`](MILO_RAG.md)).

**HTTP FAQ Milo** ([`POST /api/milo/ask`](../backend/PawBuck.API/Controllers/MiloController.cs)) uses **PawBuck.API** [`MiloRagService`](../backend/PawBuck.API/Services/MiloRagService.cs) over the **`documentation`** table and **`match_documentation`** RPC (768-dim embeddings). It is **not** wired to the consumer chat UI today; suitable for partners, admin, or future clients.

### Knowledge stores (explicit split)

| Store | RPC / access | Embedding | Used by |
|-------|----------------|-----------|---------|
| **`faq_documents`** | `match_documents` | 1536-dim (Edge embed) | Edge `search_faqs` in **`milo-chat`** |
| **`documentation`** | `match_documentation` | 768-dim (Gemini text-embedding-004, .NET) | **PawBuck.API** Milo RAG |
...
- **User/pet row from Postgres only** → new **`milo-chat`** tool + thin query (mirror [`tools/health.ts`](../apps/consumer-app/supabase/functions/milo-chat/tools/health.ts)).
- **New short FAQ for in-app chat** → `faq_source` / `faq_documents` pipeline (Edge).
- **New long-form KB chunk for API RAG** → `documentation` + API ingestion.
- **Breed-/species-grounded facts** (`weight_range`, etc.) → **`milo_curated_snippets`** + `get_curated_pet_guidance` tool; **cite only numbers present in tool output** (system prompt in `milo-chat`).

**Principle:** Edge stays the **chat orchestrator** beside Supabase; **PawBuck.API** owns **shared retrieval** and internal HTTP surfaces; avoid duplicating the same content across both vector pipelines.
