...
-- Policies: drop first so this migration is safe if the remote already had them (e.g. applied from legacy path).
DROP POLICY IF EXISTS "Users can view their own email delete audit" ON public.email_delete_audit;
DROP POLICY IF EXISTS "Users can insert audit for their threads" ON public.email_delete_audit;
DROP POLICY IF EXISTS "Service role can insert email delete audit" ON public.email_delete_audit;
