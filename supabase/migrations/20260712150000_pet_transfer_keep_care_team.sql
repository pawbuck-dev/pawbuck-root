-- Pet transfer: keep care team links on the pet for the new owner.
-- When a contact is still linked to the sender's other pets, fork vet_information so each
-- owner can unlink independently without affecting the other.
...
BEGIN
  ...
  -- Transfer care team: keep links on the pet. Fork vet_information when the prior owner
  -- still uses the same contact on another pet (so unlink/delete on either side is independent).
  FOR v_link IN
    SELECT
      pctm.id AS link_id,
...
COMMENT ON FUNCTION public.accept_pet_transfer (text, text) IS
  'Accept pet ownership transfer. Keeps care team links on the pet; forks shared vet contacts when the sender still links them on other pets.';
